...
export function NuevaVentaPage() {
  ...
  // Datos requeridos solamente para transferencia.
  const [
    nombreTransferencia,
  ...
  const [
    apellidoTransferencia,
    setApellidoTransferencia,
  ] = useState("");

  const [
    telefonoTransferencia,
    setTelefonoTransferencia,
  ] = useState("");

  // Se utiliza para Fast Cred y Finan Ya.
  const [
    pagoExternoConfirmado,
    setPagoExternoConfirmado,
  ] = useState(false);
  ...
  const yaAgregada = (id: number) =>
    items.some(
      (item) => item.variante.id === id,
    );

  const requierePagoExterno =
    medioPago === "FAST_CRED" ||
    medioPago === "FINAN_YA";

  const urlPagoExterno =
    medioPago === "FAST_CRED"
      ? import.meta.env.VITE_FAST_CRED_URL
      : medioPago === "FINAN_YA"
        ? import.meta.env.VITE_FINAN_YA_URL
        : null;

  const nombrePagoExterno =
    medioPago === "FAST_CRED"
      ? "Fast Cred"
      : medioPago === "FINAN_YA"
        ? "Finan Ya"
  ...
  const transferenciaIncompleta =
    medioPago === "TRANSFERENCIA" &&
    (
      !nombreTransferencia.trim() ||
      !apellidoTransferencia.trim() ||
      !telefonoTransferencia.trim()
    );
  ...
  async function buscarProducto(
    ...
    try {
      const encontrado =
        await productosService.findByCodigo(
          codigo,
        );
      ...
      if (!encontrado) {
        setErrorBusqueda(
          new Error(
            `No se encontró un producto con el código "${codigo.trim()}".`,
          ),
        );
      ...
      toast.success(
        ...
        }`,
        {
          description:
            medioPago ===
            "TRANSFERENCIA"
              ? "La venta fue registrada. La transferencia quedó pendiente de verificación."
              : "El backend descontó el stock y registró el movimiento.",
        },
      );
          ...
          <CardContent className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  // Si dejamos de usar transferencia,
              ...
              <div className="space-y-4 rounded-xl border border-border bg-muted/30 p-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Guardá los datos del
                    cliente para que el
                    pago pueda verificarse
                    posteriormente.
                  </p>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="nombre-transferencia">
                      Nombre
                    </Label>

                    <Input
                      id="nombre-transferencia"
                      value={
                        nombreTransferencia
                      }
                      onChange={(e) =>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="apellido-transferencia">
                      Apellido
                    </Label>

                    <Input
                      id="apellido-transferencia"
                      value={
                        apellidoTransferencia
                      }
                      onChange={(e) =>
                  ...
                  <div className="space-y-2 sm:col-span-2">
                    <Label htmlFor="telefono-transferencia">
                      Teléfono
                    </Label>

                    <Input
                      id="telefono-transferencia"
                      type="tel"
                      value={
                        telefonoTransferencia
                      }
            ...
            {requierePagoExterno ? (
              <div className="space-y-4 rounded-xl border border-primary/30 bg-primary/10 p-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <Label
                    htmlFor="pago-externo-confirmado"
                    className="cursor-pointer text-sm leading-5"
                  >
                    Confirmo que el pago
                    fue procesado
                    correctamente en{" "}
                    {nombrePagoExterno}.
                  </Label>
            ...
            {transferenciaIncompleta ? (
              <p className="text-center text-xs text-muted-foreground">
                ...
                registrar una venta por
                transferencia.
              </p>
            ) : (
              <p className="text-center text-xs text-muted-foreground">
                El descuento de stock y
                el movimiento de venta
                los realiza el backend.
              </p>
            )}
          </CardContent>
        </Card>
