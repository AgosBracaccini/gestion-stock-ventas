    ...
    <>
              ...
              {isLoading
                ...
                : (ventas ?? []).map((venta) => (
                    <TableRow key={venta.id}>
                      ...
                      <TableCell>
                        <Badge variant="secondary">{labelMedio(venta.medio_pago)}</Badge>
                      </TableCell>
                      <TableCell className="text-right font-medium">
                        {formatMoney(venta.total)}
                      </TableCell>
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" onClick={() => setSeleccionada(venta)}>
                          Ver
                        </Button>
                      </TableCell>
                    </TableRow>
      ...
      <Dialog open={!!seleccionada} onOpenChange={(open) => !open && setSeleccionada(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              Venta #{seleccionada?.id} · {formatDate(seleccionada?.fecha)}
            </DialogTitle>
          </DialogHeader>
          {seleccionada?.detalles?.length ? (
                ...
                <TableBody>
                  {seleccionada.detalles.map((detalle) => (
                    <TableRow key={detalle.id}>
                      <TableCell>
                        {detalle.codigo ?? "—"} {detalle.prenda ?? ""} {detalle.modelo ?? ""}
                      </TableCell>
                      <TableCell>{detalle.color ?? "—"}</TableCell>
                      <TableCell>{detalle.talle ?? "—"}</TableCell>
            ...
            <p className="py-6 text-sm text-muted-foreground">
              Esta venta no incluye detalles en la respuesta de la API.
            </p>
          )}
          <div className="flex justify-between border-t border-border pt-4 text-sm">
            <span className="text-muted-foreground">
              Medio de pago: {labelMedio(seleccionada?.medio_pago ?? "")}
            </span>
            <span className="font-display text-xl">{formatMoney(seleccionada?.total)}</span>
          </div>
        </DialogContent>
      </Dialog>
    </>
